...
# Step 3: Apply CURE Clustering
def apply_cure_clustering(state: DataState) -> dict:
...
def convert_to_vectors(state: DataState) -> dict:
    ...
    if os.path.exists(faiss_index_path):
        ...
        for batch_start in range(0, total_rows, batch_size):
            ...
            documents = [Document(page_content=" | ".join(map(str, row)), metadata={"index": index}) for index, row in
                         batch_df.iterrows()]
